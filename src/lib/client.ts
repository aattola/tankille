 ...
 * ```
 *
 * You can optionally pass a custom API URL to override the default endpoint (proxy for example) ;) :
 * ```ts
...
class Client {
  ...
  async getStations() {
    await this.#auth();
    const res = await this.axios.get<Asema[]>(`/stations`, {
      headers: {
        ...this.headers,
  ...
  async getStationsByLocation(
    ...
    const res = await this.axios.get<Asema[]>(
  ...
  async getStation(stationId: string, date: Duration = { days: 14 }): Promise<Asema> {
    ...
    const res = await this.axios.get<Asema>(`/stations/${stationId}/prices?since=${since}`, {
      headers: {
        ...this.headers,
        'x-access-token': this.token,
      },
